						...
						if match:
							for line in ifile:
								...
								if match:
									...
									match=re.search(r'^\[ bonds \]', line,flags=re.MULTILINE)
									if match:
										for line in ifile:
											match=re.search(r'^;', line,flags=re.MULTILINE)
											if match:
												...
												match=re.search(r'^\s*(\d+)\s+(\d+)', line,flags=re.MULTILINE)
												if match:
													a=int(match.group(1))+int(offset)
													b=int(match.group(2))+int(offset)
													if a>b:
														c=b
														b=a
														a=c
													...
													for element in connlist:
														if int(element[0])==a:
															if int(b) not in arr(element).astype(int):
																element.append(int(b))
																found=True
													if not found:
														connlist.append([int(a),int(b)])
												match=re.search(r'^\s*\n', line,flags=re.MULTILINE)
												if match:
													break
										break
...
def make_xyzq(geo,chargevec):
	...
	for element in chargevec:
		xyzq.append([float(geo[count*3+0]),float(geo[count*3+1]),float(geo[count*3+2]),float(element)])
		count+=1
	return xyzq
...
def read_pathparams(inp):
	...
	info=["","","","gromacs-2019.1/bin/","rung16","","","gmx19","gromacs-2019.1/bin/","gromacs-2019.1/share/gromacs/top/"] 
...
def read_qmparams(inp):
	import re
	info=["G16","BP86","STO-3G",int(0),int(1),int(1),int(1000),"NONE"]#program method basis charge multiplicity cores memory(MB) extraopts
	with open(inp) as ifile:
		for line in ifile:
...
def get_pcf_self_grad(charges):
	...
	for i in range(0,len(charges)-1):
		grad_line=arr([0.0,0.0,0.0])
		curr_vec=arr([float(charges[i][0]),float(charges[i][1]),float(charges[i][2])])
		curr_charge=float(charges[i][3])
		for j in range(i+1,len(charges)):
			new_vec=arr([float(charges[j][0]),float(charges[j][1]),float(charges[j][2])])
			new_charge=float(charges[j][3])
			diff=arr(new_vec-curr_vec)
			dist=LA.norm(diff)
			for k in range(0,3):
				grad_line[k]+=(curr_charge*new_charge/(dist*dist))*(diff[k]/dist)
		charges_grad.append(grad_line)
	return arr(charges_grad)
...
def read_charges_clean(file):
	...
	with open(file) as ifile:
		for line in ifile:
			...
			if match:
				clean_charges.append([float(match.group(1)),float(match.group(2)),float(match.group(3)),float(match.group(4))])
...
def write_charge_field(charges,outname):
	with open(outname,"w") as ofile:
		for element in charges:
			ofile.write("{:<.10f} {:<.10f} {:<.10f} {:<.10f}\n".format(float(element[0]),float(element[1]),float(element[2]),float(element[3])))
		ofile.write("$end\n")
...
def get_linkatoms_ang(xyzq,qmatomlist,m1list,connlist,prev_scalefacs):
	...
	for entry in m1list:
		...
		for i in range(0,len(connlist)):
			if int(entry) in arr(connlist[i]).astype(int):
				if (int(entry)==int(connlist[i][0])):
					for j in range(0,len(connlist[i])):
						if int(connlist[i][j]) in arr(qmatomlist).astype(int):
							pair=[int(connlist[i][j]),int(entry)]
							pairlist.append(pair)
							break
					break
				else:
					if int(connlist[i][0]) in arr(qmatomlist).astype(int):
	...
	for entry in pairlist:
		linkcoords=[]
		q=[float(xyzq[entry[0]-1][0]),float(xyzq[entry[0]-1][1]),float(xyzq[entry[0]-1][2])]
		m=[float(xyzq[entry[1]-1][0]),float(xyzq[entry[1]-1][1]),float(xyzq[entry[1]-1][2])]
		qmvec=arr(m)-arr(q)
		...
		qmvec_norm_scale=arr(qmvec)*scalefac
		linkcoords=[float(qmvec_norm_scale[0])+float(q[0]),float(qmvec_norm_scale[1])+float(q[1]),float(qmvec_norm_scale[2])+float(q[2]),float(scalefac)]
...
def get_linkcorrlist(linkatoms,qmatomlist,m1list,m2list,connlist):
	...
	for element in m1list:
		...
		for entry in connlist:
			if int(element) in arr(entry).astype(int):
				if int(element)!=int(entry[0]):
					if int(entry[0]) in arr(qmatomlist).astype(int):
						q1line.append(int(entry[0]))
				else:
					for i in range(1,len(entry)):
						if int(entry[i]) in arr(qmatomlist).astype(int):
							q1line.append(int(entry[i]))
		q1list.append(q1line)
	#get q2
	q1list=flatten(q1list)
	for element in q1list:
		q2line=[]
		for conn in connlist:
			if int(element) in arr(conn).astype(int):
				if int(element)!=int(conn[0]) and (int(conn[0]) in arr(qmatomlist).astype(int)) and (int(conn[0]) not in arr(flatten(q1list)).astype(int)):
					q2line.append(int(conn[0]))
				elif int(element)==int(conn[0]):
					for i in range(1,len(conn)):
						if (int(conn[i]) in arr(qmatomlist).astype(int)) and (int(conn[i]) not in arr(flatten(q1list)).astype(int)):
	...
	for element in q2list:
		...
		for entry in element:
			q3line=[]
			for conn in connlist:
				if int(entry) in arr(conn).astype(int):
					if int(entry)!=int(conn[0]) and (int(conn[0]) in arr(qmatomlist).astype(int)) and int(conn[0]) not in arr(q1list).astype(int) and int(conn[0]) not in arr(flatten(q2list)).astype(int):
						q3line.append(int(conn[0]))
					elif int(entry)==int(conn[0]):
						for i in range(1,len(conn)):
							if int(conn[i]) in arr(qmatomlist).astype(int) and int(conn[i]) not in arr(q1list).astype(int) and int(conn[i]) not in arr(flatten(q2list)).astype(int):
	...
	for element in m2list:
		...
		for entry in element:
			...
			for conn in connlist:
				if int(entry) in arr(conn).astype(int):
					if int(entry)!=int(conn[0]) and (int(conn[0]) not in arr(qmatomlist).astype(int)) and int(conn[0]) not in arr(m1list).astype(int):
						m3line.append(int(conn[0]))
					elif int(entry)==int(conn[0]):
						for i in range(1,len(conn)):
							if int(conn[i]) not in arr(qmatomlist).astype(int) and int(conn[i]) not in arr(m1list).astype(int):
	...
	for element in m3list:
		...
		for entry in element:
			...
			for stuff in entry:
				m4line=[]
				for conn in connlist:
					if int(stuff) in arr(conn).astype(int):
						if int(stuff)!=int(conn[0]) and (int(conn[0]) not in arr(qmatomlist).astype(int)) and int(conn[0]) not in arr(m1list).astype(int):
							found=False
							for morestuff in m2list:
								if int(conn[0]) in arr(morestuff).astype(int):
									found=True
									break
							if not found:
							...
							for i in range(1,len(conn)):
								if int(conn[i]) not in arr(qmatomlist).astype(int) and int(conn[i]) not in arr(m1list).astype(int):
									found=False
									for morestuff in m2list:
										if int(conn[i]) in arr(morestuff).astype(int):
											found=True
											break
									if not found:
	...
	for element in q1list:
		...
		for stuff in flatten(m3list[count]):
			if int(element)<int(stuff):
				linkpairline.append([element,stuff])
			else:
				linkpairline.append([stuff,element])
	...
	for element in q2list:
		for entry in element:
			...
			linkcorrlist.append(linkpairline)
		count+=1
	reshaped_linkcorrlist=arr(flatten(linkcorrlist)).reshape(-1,2)
	final_linkcorrlist=[]
	for element in reshaped_linkcorrlist:
		found=False
		for i in range(0,len(final_linkcorrlist)):
			if final_linkcorrlist[i][0]==element[0] and final_linkcorrlist[i][1]==element[1]:
				found=True
				break
		if found:
			continue
		final_linkcorrlist.append(element)
	return sorted(final_linkcorrlist,key=lambda l:l[0]),q1list,q2list,q3list,m3list
...
def display_help():
	print "gmx2qmmm (GNU licenced) by Jan Philipp Goetze"
	print "gmx2qmmm is a free QM/MM interface for Gromacs. Compatible with most versions, starting from 5.X."
	print "gmx2qmmm uses an additive QM/MM scheme with a numerical charge shift procedure and correction of cut bonds via model potentials."
	print "\nSYNOPSIS"
	print "\ngmx2qmmm <options>"
	print "\nOPTIONS"
	print "\nOptions to specify input files:\n"
	print " -c      [<.g96>]           (conf.g96)"
	print "           Structure file: g96"
	print " -p      [<.top>]           (topol.top)"
	print "           Topology file"
	print " -n      [<.ndx>]           (qmatoms.ndx)"
	print "           Index file (containing only the QM atom indices)"
	print " -qm     [<.dat>]           (qm.dat)"
	print "           QM input parameters"
	print " -mm     [<.dat>]           (mm.dat)"
	print "           MM input parameters"
	print " -qmmm   [<.dat>]           (qmmm.dat)"
	print "           QM/MM input parameters"
	print " -path   [<.dat>]           (path.dat)"
	print "           QM & MM software path input parameters"
	print " -act    [<.ndx>]           (act.ndx)        (Opt.)"
	print "           Index file (containing only indices of non-frozen atoms)"
	print "\nOptions to specify output files:\n"
	print " -g      [<.log>]           (logfile)   (Opt.)"
	print "           Log file"
	print "\ngmx2qmmm reminds you: This is not an official Gromacs program, so no reminder!\n"

def find_optentry(command,options):
	workwith=options
	defaults=["-c","conf.g96","-p","topol.top","-n","qmatoms.ndx","-qm","qm.dat","-mm","mm.dat","-qmmm","qmmm.dat","-act","act.ndx","-g","logfile","-path","path.dat"]
...
def gmx2qmmm(cmd_options):
	gro,top,qmatoms,qmparams,mmparams,qmmmparams,activefile,logfile,path=read_options(cmd_options)
	...
	topprep = imp.load_source("operations", str(basedir+"/operations/generate_top.py"))
	qmmm = imp.load_source("operations", str(basedir+"/operations/qmmm.py"))
	...
	logger(logfile,"Starting the preparation of the point charge field used in the calculation.\n")
	...
	qmcoordlist,m1list,m2list,updated_chargelist=prep_pcf.prepare_pcf_for_shift_fieldsonly(xyzq,qmatomlist,qminfo[3],connlist)
	logger(logfile,"done.\n")
	logger(logfile,"Setting up link atoms...")
	linkatoms=get_linkatoms_ang(xyzq,qmatomlist,m1list,connlist,[])
	linkcorrlist,q1list,q2list,q3list,m3list=get_linkcorrlist(linkatoms,qmatomlist,m1list,m2list,connlist)
	logger(logfile,"done.\n")
	if qmmminfo[7]>0:
		jobname=str(jobname+"."+str(qmmminfo[7]))
	if not os.path.isfile(str(jobname+".pointcharges")):
		logger(logfile,"Shifting...")
		final_pcf.generate_charge_shift_fieldsonly(updated_chargelist,m1list,qmcoordlist,m2list,jobname,basedir)
	else:
		logger(logfile,"NOTE: Shifting omitted due to " + str(jobname+".pointcharges") + " being an existing file!\n")
	logger(logfile,"done.\n")
	logger(logfile,"Preparing the QM/MM top file...")
	topprep.generate_top_listsonly(top,qmatoms,qmmmtop,flaglist,q1list,q2list,q3list,m1list,m2list,m3list,basedir,logfile,pathinfo)
	logger(logfile,"done.\n")
	active=[]
	...
	qmmm.perform_job(gro,top,xyzq,connlist,qmatomlist,m1list,m2list,q1list,qmmmtop,qminfo,mminfo,qmmminfo,jobtype,linkcorrlist,flaglist,pcffile,linkatoms,active,logfile,basedir,step,pathinfo)
