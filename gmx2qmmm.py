                                            ...
                                            if match:
                                                ...
                                                if match:
                                                    ...
                                                    for element in connlist:
                                                        if int(element[0])==a:
                                                            if int(b) not in np.array(element).astype(int):
...
def get_pcf_self_grad(charges):
    ...
    for i in range(0,len(charges)-1):
        ...
        curr_vec=np.array([float(charges[i][0]),float(charges[i][1]),float(charges[i][2])])
        curr_charge=float(charges[i][3])
        for j in range(i+1,len(charges)):
            new_vec=np.arrayay([float(charges[j][0]),float(charges[j][1]),float(charges[j][2])])
...
def get_pcf_self_pot(charges):
    ...
    for i in range(0,len(charges)):
        ...
        curr_charge=float(charges[i][3])
        for j in range(i+1,len(charges)):
            new_vec=np.array([float(charges[j][0]),float(charges[j][1]),float(charges[j][2])])
            new_charge=float(charges[j][3])
            diff=np.array(new_vec-curr_vec)
            dist=np.linalg.norm(diff)
...
def get_linkcorrlist(linkatoms,qmatomlist,m1list,m2list,connlist):
    ...
    #get q2
    q1list=compiler.ast.flatten(q1list)
    for element in q1list:
        q2line=[]
        for conn in connlist:
            if int(element) in np.array(conn).astype(int):
                if int(element)!=int(conn[0]) and (int(conn[0]) in np.array(qmatomlist).astype(int)) and (int(conn[0]) not in np.array(flatten(q1list)).astype(int)):
                    q2line.append(int(conn[0]))
                elif int(element)==int(conn[0]):
                    for i in range(1,len(conn)):
                        if (int(conn[i]) in np.array(qmatomlist).astype(int)) and (int(conn[i]) not in np.array(flatten(q1list)).astype(int)):
        ...
        for entry in element:
            ...
            for conn in connlist:
                if int(entry) in np.array(conn).astype(int):
                    if int(entry)!=int(conn[0]) and (int(conn[0]) in np.array(qmatomlist).astype(int)) and int(conn[0]) not in np.array(q1list).astype(int) and int(conn[0]) not in np.array(flatten(q2list)).astype(int):
                        q3line.append(int(conn[0]))
                    elif int(entry)==int(conn[0]):
                        for i in range(1,len(conn)):
                            if int(conn[i]) in np.array(qmatomlist).astype(int) and int(conn[i]) not in np.array(q1list).astype(int) and int(conn[i]) not in arr(flatten(q2list)).astype(int):
    ...
    for element in m2list:
        ...
        for entry in element:
            ...
            for conn in connlist:
                if int(entry) in np.array(conn).astype(int):
                    if int(entry)!=int(conn[0]) and (int(conn[0]) not in np.array(qmatomlist).astype(int)) and int(conn[0]) not in np.array(m1list).astype(int):
                        m3line.append(int(conn[0]))
                    elif int(entry)==int(conn[0]):
                        for i in range(1,len(conn)):
                            if int(conn[i]) not in np.array(qmatomlist).astype(int) and int(conn[i]) not in np.array(m1list).astype(int):
            ...
            for stuff in entry:
                ...
                for conn in connlist:
                    if int(stuff) in np.array(conn).astype(int):
                        if int(stuff)!=int(conn[0]) and (int(conn[0]) not in np.array(qmatomlist).astype(int)) and int(conn[0]) not in np.array(m1list).astype(int):
                            found=False
                            ...
                            for i in range(1,len(conn)):
                                if int(conn[i]) not in np.array(qmatomlist).astype(int) and int(conn[i]) not in np.array(m1list).astype(int):
                                    found=False
                                    for morestuff in m2list:
                                        if int(conn[i]) in np.array(morestuff).astype(int):
                                            found=True
                                            break
                                    if not found:
