	...
	with open(gro) as ifile:
		...
		for line in ifile:
			...
			if counter==int(n_center):
				match=re.search(r'^\s*\d+\S+\s+\S+\s*\d+\s*([-]*\d+\.\d{3})\s*([-]*\d+\.\d{3})\s*([-]*\d+\.\d{3})', line,flags=re.MULTILINE)
				if not match:
					print "Something is wrong with your .gro file. Maybe the format is unexpected? Exiting. Last line:"
					print line
					exit(1)
				coords=[float(match.group(1))*10.0,float(match.group(2))*10.0,float(match.group(3))*10.0]
				break
	return coords
	...
	with open(gro) as ifile:
		...
		for line in ifile:
			...
			if counter>0 and counter<=n_a:
				match=re.search(r'^(.{5})(.{5})(.{5})\s*\d+\s*([-]*\d+\.\d{3})\s*([-]*\d+\.\d{3})\s*([-]*\d+\.\d{3})', line,flags=re.MULTILINE)
				if not match:
					print "Something is wrong with your .gro file. Maybe the format is unexpected? Exiting. Last line:"
					print line
					exit(1)
				resline=[int(match.group(1)),(match.group(2)).strip(),(match.group(3)).strip()]
				residues.append(resline)
				coordline=[float(match.group(4))*10.0,float(match.group(5))*10.0,float(match.group(6))*10.0]
			...
			if counter>n_a:
				...
				if not match:
					...
					print line
					exit(1)
				box_vectors=[float(match.group(1))*10.0,float(match.group(2))*10.0,float(match.group(3))*10.0]
	
	if len(coords)!=n_a:
		print str("Expected " + str(n_a) + " atoms. Found " + str(len(n_a)) + " atoms. Exiting.")
		exit(1)
...
def check_max_r_for_all_mirror_images(coordline,box_vectors,max_r):
	...
	#print coordline
	curr_coords=arr(coordline)+arr([1.0*float(box_vectors[0]),0.0*float(box_vectors[1]),0.0*float(box_vectors[2])])
	if curr_max>LA.norm(curr_coords):
		curr_max=LA.norm(curr_coords)
	curr_coords=arr(coordline)+arr([-1.0*float(box_vectors[0]),0.0*float(box_vectors[1]),0.0*float(box_vectors[2])])
	if curr_max>LA.norm(curr_coords):
		curr_max=LA.norm(curr_coords)
	curr_coords=arr(coordline)+arr([0.0*float(box_vectors[0]),1.0*float(box_vectors[1]),0.0*float(box_vectors[2])])
	if curr_max>LA.norm(curr_coords):
		curr_max=LA.norm(curr_coords)
	curr_coords=arr(coordline)+arr([0.0*float(box_vectors[0]),-1.0*float(box_vectors[1]),0.0*float(box_vectors[2])])
	if curr_max>LA.norm(curr_coords):
		curr_max=LA.norm(curr_coords)
	curr_coords=arr(coordline)+arr([0.0*float(box_vectors[0]),0.0*float(box_vectors[1]),1.0*float(box_vectors[2])])
	if curr_max>LA.norm(curr_coords):
		curr_max=LA.norm(curr_coords)
	curr_coords=arr(coordline)+arr([0.0*float(box_vectors[0]),0.0*float(box_vectors[1]),-1.0*float(box_vectors[2])])
	if curr_max>LA.norm(curr_coords):
		curr_max=LA.norm(curr_coords)
	curr_coords=arr(coordline)+arr([1.0*float(box_vectors[0]),1.0*float(box_vectors[1]),0.0*float(box_vectors[2])])
	if curr_max>LA.norm(curr_coords):
		curr_max=LA.norm(curr_coords)
	curr_coords=arr(coordline)+arr([-1.0*float(box_vectors[0]),1.0*float(box_vectors[1]),0.0*float(box_vectors[2])])
	if curr_max>LA.norm(curr_coords):
	...
	if curr_max>LA.norm(curr_coords):
		curr_max=LA.norm(curr_coords)
	curr_coords=arr(coordline)+arr([1.0*float(box_vectors[0]),0.0*float(box_vectors[1]),1.0*float(box_vectors[2])])
	if curr_max>LA.norm(curr_coords):
		curr_max=LA.norm(curr_coords)
	curr_coords=arr(coordline)+arr([-1.0*float(box_vectors[0]),0.0*float(box_vectors[1]),1.0*float(box_vectors[2])])
	if curr_max>LA.norm(curr_coords):
	...
	if curr_max>LA.norm(curr_coords):
		curr_max=LA.norm(curr_coords)
	curr_coords=arr(coordline)+arr([0.0*float(box_vectors[0]),1.0*float(box_vectors[1]),1.0*float(box_vectors[2])])
	if curr_max>LA.norm(curr_coords):
		curr_max=LA.norm(curr_coords)
	curr_coords=arr(coordline)+arr([0.0*float(box_vectors[0]),-1.0*float(box_vectors[1]),1.0*float(box_vectors[2])])
	if curr_max>LA.norm(curr_coords):
	...
	if curr_max>LA.norm(curr_coords):
		curr_max=LA.norm(curr_coords)
	curr_coords=arr(coordline)+arr([1.0*float(box_vectors[0]),1.0*float(box_vectors[1]),1.0*float(box_vectors[2])])
	if curr_max>LA.norm(curr_coords):
		curr_max=LA.norm(curr_coords)
	curr_coords=arr(coordline)+arr([-1.0*float(box_vectors[0]),1.0*float(box_vectors[1]),1.0*float(box_vectors[2])])
	if curr_max>LA.norm(curr_coords):
...
def get_smallest_distance_to_mirror_images(coords,residues,solname,box_vectors):
	...
	if residues[len(coords)-1][0]==residues[len(coords)-2][0]:
		max_r=check_max_r_for_all_mirror_images(coords[len(coords)-1],box_vectors,max_r)
	return max_r
...
def find_max_dropsize(gro,r,center,solname):
	...
	max_r=get_smallest_distance_to_mirror_images(coords,residues,solname,box_vectors)
	if max_r<float(r):
...
def make_solvent_coords(solcoords,disp_vec,box_vectors):
	...
	for element in disp_vec:
		for stuff in solcoords:
			new_coords.append(arr(element).astype(float)*arr(box_vectors).astype(float)+arr(stuff).astype(float))
	return new_coords
...
def build_drop_gro(r,centered_coords,residues,box_vectors,solname):
	...
	while i<len(centered_coords)-1:
		...
		if residues[i][1]==solname:
			...
			for j in range(0,len(disp_vec)):
				for k in range(0,len(solcoords)):
					new_res.append([int(curr_res)+j,solname,residues[i+k-len(new_coords)+1][2]])
		...
		if residues[i][0]==residues[i+1][0]:
			...
			new_coords=check_min_r_for_all_mirror_images(centered_coords[i],box_vectors)
			new_res=[curr_res,residues[i][1],residues[i][2]]
			curr_res+=1
	...
	if residues[len(centered_coords)-1][1]!=solname:
		if residues[len(centered_coords)-1][0]==residues[len(centered_coords)-2][0]:
			drop_coords.append(centered_coords[len(centered_coords)-1])
			drop_res.append([curr_res,residues[len(residues)-1][1],residues[len(residues)-1][2]])
			curr_res+=1
		else:#is NOT a large residue
			new_coords=check_min_r_for_all_mirror_images(centered_coords[len(centered_coords)-1],box_vectors)
			drop_coords.append(new_coords)
			drop_res.append([curr_res,residues[len(residues)-1][1],residues[len(residues)-1][2]])
			curr_res+=1
	for i in range(0,3):
	...
	with open(groout,"w") as ofile:
		with open(gro) as ifile:
			...
			for line in ifile:
				match=re.search(r'^\s*(\d+\.\d*)\s*(\d+\.\d*)\s*(\d+\.\d*)', line,flags=re.MULTILINE)
				if match:
					ofile.write("{:>10.5f}{:>10.5f}{:>10.5f}".format(float(new_vectors[0])/10.0,float(new_vectors[1])/10.0,float(new_vectors[2])/10.0))
					continue
				counter+=1
				if counter==len(new_coords):
					break
				match=re.search(r'^(.{5})(.{5})(.{5})(.{5})\s*([-]*\d+\.\d{3})\s*([-]*\d+\.\d{3})\s*([-]*\d+\.\d{3})', line,flags=re.MULTILINE)
				if match:
					if (match.group(2)).strip()!=solname:
						index2=counter+1%100000
						ofile.write("{:>5d}{:<5s}{:>5s}{:>5d}{:>8.3f}{:>8.3f}{:>8.3f}\n".format(int(new_res[counter][0])%100000,match.group(2),match.group(3),index2,new_coords[counter][0]/10.0,new_coords[counter][1]/10.0,new_coords[counter][2]/10.0))
						continue
					else:
						for line in ifile:
							match=re.search(r'^\s*(\d+\.\d*)\s*(\d+\.\d*)\s*(\d+\.\d*)', line,flags=re.MULTILINE)
						...
						while new_res[counter][1]==solname:
							...
							ofile.write("{:>5d}{:<5s}{:>5s}{:>5d}{:>8.3f}{:>8.3f}{:>8.3f}\n".format(int(new_res[counter][0])%100000,new_res[counter][1],new_res[counter][2],index2,new_coords[counter][0]/10.0,new_coords[counter][1]/10.0,new_coords[counter][2]/10.0))
							counter+=1
					...
					for line in ifile:
						...
						if not match and not found:
							...
							for element in new_mollist:
								ofile.write("{:<20s}{:>10d}\n".format(element[0],int(element[1])))
					ofile.write("\n")
...
def create_droplet(gro,top,r,n_center,solname,groout,topout):
	...
	new_coords,new_res,new_vectors=build_drop_gro(r,centered_coords,residues,box_vectors,solname)
...
if __name__ == "__main__":
	import sys
	create_droplet(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5],sys.argv[6],sys.argv[7])
