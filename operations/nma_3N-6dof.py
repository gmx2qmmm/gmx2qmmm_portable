...
def make_stretch(curr_index,shaped_coords,basedir,n_a):
	...
	unit_fwd=rot.uvec(arr(shaped_coords[curr_index])-arr(shaped_coords[curr_index-1]))
...
def make_angle(curr_index,shaped_coords,basedir,n_a):
	...
	unit_v1=rot.uvec(arr(shaped_coords[curr_index])-arr(shaped_coords[curr_index-1]))
	unit_v2=rot.uvec(arr(shaped_coords[curr_index-2])-arr(shaped_coords[curr_index-1]))
	...
	sqrtvec=[sqr(abs(distv1[0]*distv2[0])),sqr(abs(distv1[1]*distv2[1])),sqr(abs(distv1[2]*distv2[2]))]
	if distv1[0]*distv2[0]<0.:
		sqrtvec[0]*=-1.0
	if distv1[1]*distv2[1]<0.:
		sqrtvec[1]*=-1.0
	if distv1[2]*distv2[2]<0.:
		sqrtvec[2]*=-1.0
	angleelement=(distv1-math.cos(phi)*distv2)*unit_v1+unit_v2*(distv2-distv1*math.cos(phi))/(arr(sqrtvec)*math.sin(phi))
	angleline.append([angleelement[0],angleelement[1],angleelement[2]])
...
def nma_3Nminus6dof_asfunction(hessfile,basedir):
	extr = imp.load_source("orcaextractors", str(basedir)+"/operations/hessian_from_hes.py")
	extrmass = imp.load_source("orcaextractors", str(basedir)+"/operations/mass_from_hes.py")
	extrxyz = imp.load_source("orcaextractors", str(basedir)+"/operations/geo_from_hes.py")
	...
	ele=float(0.)
	for i in range(0,len(mass)):
		ele+=float(mass[i])*(shift_coords[i*3+1]*shift_coords[i*3+1]+shift_coords[i*3+2]*shift_coords[i*3+2])
	t_row.append(ele)
	ele=float(0.)
	for i in range(0,len(mass)):
	...
	ele=float(0.)
	for i in range(0,len(mass)):
		ele+=float(mass[i])*(shift_coords[i*3+0]*shift_coords[i*3+0]+shift_coords[i*3+2]*shift_coords[i*3+2])
	t_row.append(ele)
	ele=float(0.)
	for i in range(0,len(mass)):
	...
	ele=float(0.)
	for i in range(0,len(mass)):
		ele+=float(mass[i])*(shift_coords[i*3+0]*shift_coords[i*3+0]+shift_coords[i*3+1]*shift_coords[i*3+1])
	t_row.append(ele)
	t_inert.append(t_row)
	...
	for i in range(0,len(mass)):
		...
		for j in range(0,3):
			rotvec.append((py*princ[2][j]-pz*princ[1][j])*math.sqrt(float(mass[i])))
	...
	for i in range(0,len(mass)):
		...
		pz=dot(arr(curr_coords).astype(float),arr(princ[2]).astype(float))
		for j in range(0,3):
			rotvec.append((pz*princ[0][j]-px*princ[2][j])*math.sqrt(float(mass[i])))
	transrotmat.append(rotvec)
	rotvec=[]
	for i in range(0,len(mass)):
		curr_coords=[]
		...
		py=dot(arr(curr_coords).astype(float),arr(princ[1]).astype(float))
		for j in range(0,3):
			rotvec.append((px*princ[1][j]-py*princ[0][j])*math.sqrt(float(mass[i])))
	transrotmat.append(rotvec)
	...
	for i in range(0,len(mass)):
		for j in range(0,3):
			for k in range(0,len(mass)):
				for l in range(0,3):
					hessian[i*3+j][k*3+l]/=math.sqrt(float(mass[i])*float(mass[k]))
					if hessian[i*3+j][k*3+l]<0.00000000000001 and hessian[i*3+j][k*3+l]>0.:
						hessian[i*3+j][k*3+l]=float(0.)
					if hessian[i*3+j][k*3+l]>-0.00000000000001 and hessian[i*3+j][k*3+l]<0.:
						hessian[i*3+j][k*3+l]=float(0.)
