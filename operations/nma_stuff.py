		...
		with open(gro) as ifile:
			...
			for line in ifile:
				...
				if count==int(curr_atom)-1:
					break
			for line in ifile:
				match=re.search(r'^(.{24})\s*([-]*\d+\.\d+)\s*([-]*\d+\.\d+)\s*([-]*\d+\.\d+)', line,flags=re.MULTILINE)
				if not match:
					print "Error in automatically generated file " + str(gro) + " for atom " + str(curr_atom) + ". This should never happen. Exiting. Last line:"
					print line
					exit(1)
				...
				for i in range(0,3):
					if i!=coord_index:
						...
						ofile.write("{:>16.9f}".format(float(match.group(i+2))+nm_disp))
				ofile.write("\n")
				break
			for line in ifile:
				ofile.write(line)
...
def eval_gro(gro,top,active,xyzq,connlist,qmatomlist,m1list,m2list,q1list,qmmmtop,qminfo,mminfo,qmmminfo,linkcorrlist,flaglist,pcffile,linkatoms,logfile,basedir):
	...
	if os.path.isfile(archivename):
		...
		call(["rm",archivename])
	new_xyzq,m1list,m2list,new_links=qmmm_prep(gro,top,jobname,0,qminfo,qmatomlist,connlist,linkatoms,basedir,logfile)
	#we may skip a new pcf generation if all act atoms are in qm region - for this case the pcf stays the same
	qmenergy,mmenergy,qm_corrdata=get_energy(gro,jobname,new_xyzq,connlist,qmatomlist,m1list,m2list,q1list,qmmmtop,qminfo,mminfo,qmmminfo,linkcorrlist,flaglist,str(jobname + ".pointcharges"),new_links,0,logfile,basedir)
	sp_energy=float(qmenergy)+float(mmenergy)
	curr_forces=read_forces(qmatomlist,m1list,qmmmtop,qminfo,jobname,0,logfile,linkcorrlist,new_xyzq,new_pcffile,qm_corrdata,m2list,q1list,new_links,active,basedir)
...
def log_nma(qmmminfo,logfile,evals,nm_matrix,active,qmmmtop,xyzq,hess):
	...
	write_pseudofchk_file(jobname,evals,nm_matrix,hess,active,qmmmtop,logfile,xyzq)
	logger(logfile,"Wrote new Gaussian03 style pseudo-fchk file ("+jobname+".pseudofchk).\n")
	write_hess.hes_xyz_fchk(str(jobname+".pseudofchk"),str(jobname+".hess"))
	logger(logfile,"Wrote orca format "+jobname+".hess file.\n")

def write_pseudofchk_file(jobname,evals,nm_matrix,hess,active,qmmmtop,logfile,xyzq):
	from compiler.ast import flatten
	flat_nm=flatten(nm_matrix)
	outname=jobname+".pseudofchk"
	n_a=len(active)
	dof=len(active)*3
	atom_map = {"1" : "H", "2" : "He", "3" : "Li", "4" : "Be", "5" : "B", "6" : "C", "7" : "N", "8" : "O", "9" : "F", "10" : "Ne", "11" : "Na", "12" : "Mg", "13" : "Al", "14" : "Si", "15" : "P", "16" : "S", "17" : "Cl", "18" : "Ar", "19" : "K", "20" : "Ca", "21" : "Sc", "22" : "Ti", "23" : "V", "24" : "Cr", "25" : "Mn", "26" : "Fe", "27" : "Co", "28" : "Ni", "29" : "Cu", "30" : "Zn", "31" : "Ga", "32" : "Ge", "33" : "As", "34" : "Se", "35" : "Br", "36" : "Kr", "37" : "Rb", "38" : "Sr", "39" : "Y", "40" : "Zr", "41" : "Nb", "42" : "Mo", "43" : "Tc", "44" : "Ru", "45" : "Rh", "46" : "Pd", "47" : "Ag", "48" : "Cd", "49" : "In", "50" : "Sn", "51" : "Sb", "52" : "Te", "53" : "I", "54" : "Xe", "55" : "Cs", "56" : "Ba", "57" : "La", "58" : "Ce", "59" : "Pr", "60" : "Nd", "61" : "Pm", "62" : "Sm", "63" : "Eu", "64" : "Gd", "65" : "Tb", "66" : "Dy", "67" : "Ho", "68" : "Er", "69" : "Tm", "70" : "Yb", "71" : "Lu", "72" : "Hf", "73" : "Ta", "74" : "W", "75" : "Re", "76" : "Os", "77" : "Ir", "78" : "Pt", "79" : "Au", "80" : "Hg", "81" : "Tl", "82" : "Pb", "83" : "Bi", "84" : "Po", "85" : "At", "86" : "Rn", "87" : "Fr", "88" : "Ra", "89" : "Ac", "90" : "Th", "91" : "Pa", "92" : "U", "93" : "Np", "94" : "Pu", "95" : "Am", "96" : "Cm", "97" : "Bk", "98" : "Cf", "99" : "Es", "100" : "Fm", "101" : "Md", "102" : "No", "103" : "Lr", "104" : "Rf", "105" : "Db", "106" : "Sg", "107" : "Bh", "108" : "Hs", "109" : "Mt", "110" : "Ds", "111" : "Rg", "112" : "Cn", "113" : "Nh", "114" : "Fl", "115" : "Mc", "116" : "Lv", "117" : "Ts", "118" : "Og"}
		...
		for i in range(0,(dof-6)*14):
			...
			if i>=len(evals):
				...
				if float(evals[i])<0.0:
					ofile.write(" {:>16.8E}".format(float(-1.0*sqr(abs(float(evals[i]))*float(26424608)))))
				else:
					ofile.write(" {:>16.8E}".format(float(sqr(float(evals[i])*float(26424608)))))
...
def get_xyz_2nd_deriv(curr_atom,start_energy,start_forces,gro,top,xyzq,connlist,qmatomlist,m1list,m2list,q1list,qmmmtop,qminfo,mminfo,qmmminfo,linkcorrlist,flaglist,pcffile,linkatoms,active,logfile,basedir):
	import imp
	from numpy import array as arr
	from compiler.ast import flatten
	...
	grad_vectors=[]
	logger(logfile,"------Getting second derivatives for atom " + str(curr_atom) + "------\n")
	for i in range(0,3):
		new_g96=make_disp_g96(gro,i,curr_atom,disp)
		plus_energy,plus_forces=eval_gro(new_g96,top,active,xyzq,connlist,qmatomlist,m1list,m2list,q1list,qmmmtop,qminfo,mminfo,qmmminfo,linkcorrlist,flaglist,pcffile,linkatoms,logfile,basedir)
		plus_grad=arr(plus_forces)*-1.0
		if plus_energy<start_energy:
			logger(logfile,"Note: Atom " + str(curr_atom) + " displaced by " + str(disp) + " a.u. along coordinate " + str(i) + " yields lower energy than initial point:\n")
			logger(logfile,str(float(plus_energy))+" a.u. vs. "+str(float(start_energy))+ " a.u.\n")
		new_g96=make_disp_g96(gro,i,curr_atom,-1.0*disp)
		minus_energy,minus_forces=eval_gro(new_g96,top,active,xyzq,connlist,qmatomlist,m1list,m2list,q1list,qmmmtop,qminfo,mminfo,qmmminfo,linkcorrlist,flaglist,pcffile,linkatoms,logfile,basedir)
		minus_grad=arr(minus_forces)*-1.0
		if minus_energy<start_energy:
			logger(logfile,"Note: Atom " + str(curr_atom) + " displaced by -" + str(disp) + " a.u. along coordinate " + str(i) + " yields lower energy than initial point:\n")
			logger(logfile,str(float(minus_energy))+" a.u. vs. "+str(float(start_energy))+ " a.u.\n")
		average_grad=flatten((arr(flatten(plus_grad))-arr(flatten(minus_grad)))/(float(disp)*2.0))
