...
from gmx2qmmm.generators.pcf.base import PCFGenerator, dump_field, load_field
from gmx2qmmm.generators.system import SystemInfo
from gmx2qmmm.generators.types import (
    Point,
    PointCharge,
    PointChargeField,
    Points,
...
class PCFGeneratorShift(PCFGenerator):
    ...
    def write_output(self, filename: StrPath) -> None:
        ...
        with open(filename, "w") as fp:
            dump_field(
                fp, self.output_field_, annotations=self.annotations, legacy=True
            )
    ...
    def _shift_m1_charges_to_m2(self) -> None:
        ...
        while iteration < max_iter and max_disp_change > eps:
            ...
            for i, (m1, m2) in enumerate(self.m1m2_pairs):
                ...
                if new_disp_vector[i] < min_disp:
                    new_disp_vector[i] = min_disp
