...
def geo_xyz_g09RevA02log(inpname,outname):
	...
	for line in ifile:
		...
		if match:
			found = True
			never_found = False
		match = re.search('\s+\d+\s{7}\s*(\d+)\s{7}\s*\d+\s{3}\s*([-]*\d+\.\d{6})\s*([-]*\d+\.\d{6})\s*([-]*\d+\.\d{6})', line)
		if match and found:
			if droplines:
				droplines = False
				linecount=0
			reformatted = '  {:<2} {:>18.14f}    {:>18.14f}    {:>18.14f}'.format(atom_map[match.group(1)], float(match.group(2)), float(match.group(3)), float(match.group(4)))
			linecount += 1
			buffer.append("\n"+reformatted)
		elif (not match) and (not droplines):#after a geometry
			found = False
			match = re.search('\sSCF Done:\s*E\([^\s]+\)\s=\s*([-]*\d+\.\d+)\s*A\.U\.\safter', line)
			if match:
				ofile = open(outname, "w")
				ofile.write(str(linecount)+"\nCoordinates from G09 job "+inpname+" E "+match.group(1))
		...
		for line in ifile:
			...
			if match and found:
				if droplines:
					droplines = False
					linecount = 0
				reformatted = '  {:<2} {:>18.14f}    {:>18.14f}    {:>18.14f}'.format(atom_map[match.group(1)], match.group(2), match.group(3), match.group(4))
				linecount += 1
				buffer.append("\n" + reformatted)
			elif (not match) and (not droplines):#after a geometry
				found = False
