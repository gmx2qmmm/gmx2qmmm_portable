...
from gmx2qmmm.operations.qmmm_job import perform_sp, qmmm_prep
...
def eval_gro(qmmmInputs):
    ...
    sp_energy = qmmmInputs.energies[-1]
    curr_forces = qmmmInputs.forces
...
def get_xyz_2nd_deriv(qmmmInputs, curr_atom, start_energy, start_forces,):
    logfile = qmmmInputs.qmmmparams.logfile
    gro = qmmmInputs.gro
    # DO FOR 3 COORDS AND PLUS AND MINUS
    disp = qmmmInputs.qmmmparams.disp
    grad_vectors = []
    logger(
        logfile,
    ...
    for i in range(0, 3):
        ...
        plus_energy, plus_forces = eval_gro(qmmmInputs)
        ...
        minus_energy, minus_forces = eval_gro(qmmmInputs)
