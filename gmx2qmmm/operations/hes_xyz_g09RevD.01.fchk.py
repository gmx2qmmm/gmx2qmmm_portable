...
def hes_xyz_fchk(inpname, outname):
		...
		if match:
			...
			for line in ifile:
				...
				for i in range(0, len(splitline)):
					if math.sqrt(float(splitline[i])*float(splitline[i])) < 0.00000000000001:
	...
	while rem_cols > 0:
		...
		for i in range(0, n_dof):
			...
			for j in range(0, col_limit):
				colstring += '  {:>17.10E}'.format(float(list_h_orca[i*n_dof+curr_col+j]))
			ofile.write(colstring+"\n")
		curr_col += 5
		rem_cols -= 5
	...
	for i in range(0,n_dof):
		for j in range(0,n_dof):
			if j < 6:
				list_nm_orca[i*n_dof+j]=0.0000000000E+00
			else:
	...
	while rem_cols > 0:
		...
		for i in range(0, n_dof):
			colstring = '{:>5}   '.format(i)
			for j in range(0, col_limit):
				colstring += '  {:>17.10E}'.format(float(list_nm_orca[i*n_dof+curr_col+j]))
			ofile.write(colstring+"\n")
		curr_col += 5
		rem_cols -= 5
	...
	for i in range(0, int(n_a)):
		reformatted = ' {:<2} {:>11.5f}  {:>18.12f} {:>18.12f} {:>18.12f}\n'.format(atom_map[list_i_a[i]], float(list_mass[i]), float(list_i_c[3*i+0])/0.529177, float(list_i_c[3*i+1])/0.529177, float(list_i_c[3*i+2])/0.529177)
	...
	for i in range(0, int(n_dip)/3):
		reformatted = '{:>20.10E}{:>20.10E}{:>20.10E}\n'.format(float(list_dip[i*3+0]), float(list_dip[i*3+1]), float(list_dip[i*3+2]))
		ofile.write(reformatted)
	ofile.write("\n#\n# The IR spectrum\n#  wavenumber T**2 TX TY  TY\n#\n$ir_spectrum\n"+str(n_dof)+"\n")
	for i in range(0, 6):
		ofile.write("      0.00       0.0000       0.0000       0.0000       0.0000\n")
	for i in range(0, n_v):
		reformatted = '{:>10.2f}{:>13.4f}{:>13}{:>13}{:>13}'.format(list_v[i], list_v[3*n_v+i], "tbd", "tbd", "tbd")
		ofile.write(reformatted+"\n")
	ofile.write("\n\n$end\n\n")
	ifile.close()
	ofile.close()
