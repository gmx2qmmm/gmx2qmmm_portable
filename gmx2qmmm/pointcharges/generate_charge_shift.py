...
def create_corr_charges(m1coordsq,m2coordsqlist,disp_vec,disp_charge_vec,m2_nolist):
	...
	for i in range(0,len(m2_nolist)):
		...
		charge_list=write_disp_charges(m1coordsq[i],m2coordsqlist[i],short_disp_vec,disp_charge_vec[i])
		corr_charge_list.extend(charge_list)
		count+=len(m2_nolist[i])
	corr_charge=arr(corr_charge_list).reshape(-1,4)
	return corr_charge
...
def write_disp_charges(m1,m2coordsqlist,disp_vec,dispcharge):
	...
	for i in range(0,len(m2coordsqlist)):
		ab_vec=arr([float(m2coordsqlist[i][0]),float(m2coordsqlist[i][1]),float(m2coordsqlist[i][2])])-arr([float(m1[0]),float(m1[1]),float(m1[2])])
		length_ab=LA.norm(ab_vec)
		normvec=uvec(ab_vec)
		ab_long=arr(normvec)*(length_ab+disp_vec[i])
		ab_short=arr(normvec)*(length_ab-disp_vec[i])
		new_dispcharge=0.0
		if disp_vec[i]>=0.000001:
			new_dispcharge=0.5-(0.5/(2.0*disp_vec[i]+1.0))
		if dispcharge>0.:
			chargeminus=[float(m1[0])+float(ab_long[0]),float(m1[1])+float(ab_long[1]),float(m1[2])+float(ab_long[2]),float(-1.*new_dispcharge)]
			chargeplus=[float(m1[0])+float(ab_short[0]),float(m1[1])+float(ab_short[1]),float(m1[2])+float(ab_short[2]),float(new_dispcharge)]
		else:
			chargeplus=[float(m1[0])+float(ab_long[0]),float(m1[1])+float(ab_long[1]),float(m1[2])+float(ab_long[2]),float(new_dispcharge)]
			chargeminus=[float(m1[0])+float(ab_short[0]),float(m1[1])+float(ab_short[1]),float(m1[2])+float(ab_short[2]),float(-1.*new_dispcharge)]
		...
		for line in ifile:
			...
			if int(count) in m2list:
				for i in range(0,len(m2list)):
					if int(count)==int(m2list[i]):
						ofile.write(str(m2atoms[i][0]) + " " + str(m2atoms[i][1]) + " " + str(m2atoms[i][2]) + " " + str(m2atoms[i][3]) + "\n")
						continue
			...
			if match:
				for i in range(0,len(m2list)):
					...
					ab_short=arr(normvec)*(length_ab-disp_vec[i])
					if distrib_charge>0.:
						ofile.write(str(float(m1[0])+float(ab_long[0])) + " " + str(float(m1[1])+float(ab_long[1])) + " " + str(float(m1[2])+float(ab_long[2])) + " " + str(dispcharge) + "\n")
						ofile.write(str(float(m1[0])+float(ab_short[0])) + " " + str(float(m1[1])+float(ab_short[1])) + " " + str(float(m1[2])+float(ab_short[2])) + " " + str(-1.*dispcharge) + "\n")
	
					else:
						ofile.write(str(float(m1[0])+float(ab_long[0])) + " " + str(float(m1[1])+float(ab_long[1])) + " " + str(float(m1[2])+float(ab_long[2])) + " " + str(-1*dispcharge) + "\n")
						ofile.write(str(float(m1[0])+float(ab_short[0])) + " " + str(float(m1[1])+float(ab_short[1])) + " " + str(float(m1[2])+float(ab_short[2])) + " " + str(dispcharge) + "\n")
				ofile.write("$end\n")
			else:
				ofile.write(line)
		ofile.close()
...
def write_new_field_to_disk_listsonly(inp,ofilename,new_field,getlist,m2_nolist):
	...
	count=0
	m2list=arr(m2_nolist).reshape(-1)
	m2count=0
	for element in inp:
		count+=1
		if int(count) in arr(flatten(m2list)).astype(int):
			...
			m2count+=1
			continue
		if int(count) in arr(getlist).astype(int):
			ofile.write("QM\n")
			continue
		else:
			for i in range(0,len(element)):
				...
				if i!=len(element)-1:
					ofile.write(" ")
			ofile.write("\n")
	for i in range(len(flatten(m2list)),len(new_field)):
		ofile.write("{:<.10f} {:<.10f} {:<.10f} {:<.10f}\n".format(float(new_field[i][0]),float(new_field[i][1]),float(new_field[i][2]),float(new_field[i][3])))
	ofile.write("$end\n")
	ofile.close()
			...
			if count in m2list.astype(int):
				...
				for i in range(0,len(m2list)):
					if count==int(m2list[i]):
						m2pos=i
						break
				ofile.write("{:<.10f} {:<.10f} {:<.10f} {:<.10f}\n".format(float(match.group(1)),float(match.group(2)),float(match.group(3)),float(new_field[i][3])+float(match.group(4))))
				continue
			if int(count) in arr(getlist).astype(int):
				ofile.write("QM\n")
				continue
...
def get_qmlist(inp):
	...
	with open(inp) as ifile:
		for line in ifile:
			match=re.search(r'^\s*([-]*\d+\.\d+[e]*[-]*[+]*\d*)\s+([-]*\d+\.\d+[e]*[-]*[+]*\d*)\s+([-]*\d+\.\d+[e]*[-]*[+]*\d*)', line,flags=re.MULTILINE)
			if match:
				xyz.append([float(match.group(1)),float(match.group(2)),float(match.group(3))])
	return xyz
...
def generate_charge_shift_fieldsonly(pcf,m1list,qmcoords,m2list,jobname,basedir):
	...
	orgfield=[]
	qmlist=qmcoords
	make_p_sum = imp.load_source("operations", str(basedir+"/pointcharges/sum_pcf_tm.py"))
	target_sum=arr([0.0,0.0,0.0])
	...
	for element in qmlist:
		target_sum+=arr(make_p_sum.sum_pcf_tm_nofile(orgfield,element[0],element[1],element[2]))
	...
	for element in m2list:
		...
		effect_dispcharge=float(dispcharge)*(m1[3]/float(len(element))/0.023333333333)
	...
	corr_charge_list=create_corr_charges(m1coordsq,m2coordsqlist,disp_vec,disp_charge_vec,m2_nolist)

	new_field=arr(make_new_field(m2coordsqlist,corr_charge_list))
	curr_sum=arr([0.0,0.0,0.0])
	for element in qmlist:
		curr_sum+=arr(make_p_sum.sum_pcf_tm_nofile(new_field,element[0],element[1],element[2]))

	curr_delta=math.sqrt((curr_sum[0]-target_sum[0])*(curr_sum[0]-target_sum[0])+(curr_sum[1]-target_sum[1])*(curr_sum[1]-target_sum[1])+(curr_sum[2]-target_sum[2])*(curr_sum[2]-target_sum[2]))
	max_change_disp=disp
	change_disp=[]
	change_disp=arr(disp_vec)
	startfac=10.
	v_opt_tracker=[]
	...
	while max_change_disp>0.0000001 and iter_count<200:
		...
		for i in range(0,len(m2_nolist)):
			for j in range(0,len(m2_nolist[i])):
				...
				if change_disp[count]<=0.00001:
					count+=1
					continue
				new_disp_vec=arr(disp_vec)
				new_disp_vec[count]+=change_disp[count]
				curr_corr_charge_list=create_corr_charges(m1coordsq,m2coordsqlist,new_disp_vec,disp_charge_vec,m2_nolist)
				curr_field=arr(make_new_field(m2coordsqlist,curr_corr_charge_list))
				new_sum=arr([0.0,0.0,0.0])
				for element in qmlist:
					new_sum+=arr(make_p_sum.sum_pcf_tm_nofile(curr_field,element[0],element[1],element[2]))
				v_opt_tracker[count][0]=math.sqrt((new_sum[0]-target_sum[0])*(new_sum[0]-target_sum[0])+(new_sum[1]-target_sum[1])*(new_sum[1]-target_sum[1])+(new_sum[2]-target_sum[2])*(new_sum[2]-target_sum[2]))
				new_disp_vec=arr(disp_vec)
				new_disp_vec[count]-=change_disp[count]
				curr_corr_charge_list=create_corr_charges(m1coordsq,m2coordsqlist,new_disp_vec,disp_charge_vec,m2_nolist)
				curr_field=arr(make_new_field(m2coordsqlist,curr_corr_charge_list))
				...
				for element in qmlist:
					new_sum+=arr(make_p_sum.sum_pcf_tm_nofile(curr_field,element[0],element[1],element[2]))
				v_opt_tracker[count][1]=math.sqrt((new_sum[0]-target_sum[0])*(new_sum[0]-target_sum[0])+(new_sum[1]-target_sum[1])*(new_sum[1]-target_sum[1])+(new_sum[2]-target_sum[2])*(new_sum[2]-target_sum[2]))
				if (float(v_opt_tracker[count][1])<float(curr_delta)) and (float(v_opt_tracker[count][1])<float(v_opt_tracker[count][0])):
					mod_disp_vec[count]-=change_disp[count]
					if (math.sqrt(mod_disp_vec[count]*mod_disp_vec[count]))<0.000001:
						mod_disp_vec[count]=0.000001
					v_opt_tracker[count][2]=True
				elif (float(v_opt_tracker[count][0])<float(curr_delta)) and (float(v_opt_tracker[count][0])<float(v_opt_tracker[count][1])):
					mod_disp_vec[count]+=change_disp[count]
					if (math.sqrt(mod_disp_vec[count]*mod_disp_vec[count]))<0.000001:
						mod_disp_vec[count]=0.000001
					v_opt_tracker[count][2]=True
		...
		new_disp_vec=arr(mod_disp_vec)
		curr_corr_charge_list=create_corr_charges(m1coordsq,m2coordsqlist,new_disp_vec,disp_charge_vec,m2_nolist)
		curr_field=arr(make_new_field(m2coordsqlist,curr_corr_charge_list))
		new_sum=arr([0.0,0.0,0.0])
		for element in qmlist:
			new_sum+=arr(make_p_sum.sum_pcf_tm_nofile(curr_field,element[0],element[1],element[2]))
		new_delta=math.sqrt((new_sum[0]-target_sum[0])*(new_sum[0]-target_sum[0])+(new_sum[1]-target_sum[1])*(new_sum[1]-target_sum[1])+(new_sum[2]-target_sum[2])*(new_sum[2]-target_sum[2]))
	...
	new_sum=arr([0.0,0.0,0.0])
	for element in qmlist:
		new_sum+=arr(make_p_sum.sum_pcf_tm_nofile(new_field,element[0],element[1],element[2]))
	new_delta=math.sqrt((new_sum[0]-target_sum[0])*(new_sum[0]-target_sum[0])+(new_sum[1]-target_sum[1])*(new_sum[1]-target_sum[1])+(new_sum[2]-target_sum[2])*(new_sum[2]-target_sum[2]))
...
def generate_charge_shift(syscmds):
	...
	with open(m2listlist) as ifile:
		for line in ifile:
			...
			disp_charge_vec.append(float(dispcharge)*(m1[3]/float(len(m2_no))/0.023333333333))
			count+=1
