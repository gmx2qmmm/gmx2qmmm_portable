...
def identify_m1(qmlist,connlist):
	...
	for element in qmlist:
		bondlist=get_bondpartners(connlist,element)
		for entry in bondlist:
			if (int(entry) not in arr(qmlist).astype(int)) and (int(entry) not in arr(m1list).astype(int)):
				m1list.append(entry)
	return m1list
...
def get_qmcoords(qmatoms,charges):
	...
	for element in qmatoms:
		qmline=[float(charges[int(element)-1][0]),float(charges[int(element)-1][1]),float(charges[int(element)-1][2]),float(charges[int(element)-1][3])]
...
def read_charge_list(inp):
	...
	with open(inp) as ifile:
		for line in ifile:
			...
			if match:
				chargeline=[float(match.group(1)),float(match.group(2)),float(match.group(3)),float(match.group(4))]
...
def eliminate_and_shift_to_m1(qmatoms,charges,m1list,qmcharge,qmcoordsq):
	...
	for element in updated_charges:
		...
		if count in arr(m1list).astype(int):
			updated_charges[count-1][3]=float(updated_charges[count-1][3])+float(parentcharge)
...
def prepare_pcf_for_shift_fieldsonly(charges,qmatomlist,qmcharge,connlist):
	...
	updated_chargelist=eliminate_and_shift_to_m1(qmatomlist,charges,m1list,qmcharge,qmcoordlist)
	return qmcoordlist,m1list,m2list,updated_chargelist

def prepare_pcf_for_shift(inp,qmatoms,qmcharge,connfile,qmcoords,m1file,m2file,outfile):
	import re
	from numpy import array as arr
	...
	updated_chargelist=eliminate_and_shift_to_m1(qmatomlist,chargelist,m1list,qmcharge,qmcoordlist)
	with open(qmcoords,"w") as ofile:
		for element in arr(qmcoordlist):
			ofile.write("{:<.10f} {:<.10f} {:<.10f}\n".format(float(element[0]),float(element[1]),float(element[2])))
